...
public class PlayerAttacks : MonoBehaviour
{
    ...
    void Update()
    {
        Check = GetComponent<MovementScript>().jumping;
        recharging = GetComponent<MovementScript>().canRecharge;
        // //Debug.Log("Next Attack: " + nextAttackTime + ", canAttack: " + canAttack);
        if (Time.time >= nextAttackTime)
        {
            if (canAttack)
            {
                if ((Input.GetMouseButtonDown(0)) || (Input.GetKey("z")) && !Check && currentStamina > 0f )
                {
